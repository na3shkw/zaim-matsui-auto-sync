...
ログイン方式は `MATSUI_LOGIN_METHOD` で選択できる。

| 値 | 説明 |
|---|---|
| `password` | パスワード + SMS 認証によるログイン（デフォルト） |
| `passkey` | パスキーによるログイン |

### パスキーの登録（パスキーログイン方式を使う場合）

パスキーログイン方式を利用する場合は、事前に下記の手順でパスキーを登録して資格情報を保存する。
パスワードログイン方式を使う場合はこの手順は不要。

1. `.env` の `MATSUI_LOGIN_METHOD=passkey` を設定する。
2. 下記のコマンドでパスキー登録ツールを起動する。ヘッドレスモードでは実行できないため VNC 接続が必要。
    ```bash
    docker compose run --rm -e APP_COMMAND='register-matsui-passkey' -e HEADLESS=false --publish <ホスト側 VNC ポート>:5901 app
    ```
3. `localhost:<ホスト側 VNC ポート>` に VNC 接続する。
4. ブラウザで松井証券お客様サイト > セキュリティ設定 > パスキー登録 からパスキーを登録する。
5. 登録が完了したらターミナルで Enter を押す。

### Google へのログイン（パスワードログイン方式を使う場合）
...
1. 下記のコマンドでパスキー登録ツールを起動する。
    ```bash
    HEADLESS=false npm run register-passkey
    ```
2. ブラウザで松井証券お客様サイト > セキュリティ設定 > パスキー登録 からパスキーを登録する。
3. 登録が完了したらターミナルで Enter を押す。
